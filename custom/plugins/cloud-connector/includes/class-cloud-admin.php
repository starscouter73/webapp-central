...
final class CloudAdmin
{
    ...
    private static function renderExplorer(): void
    {
        ...
        foreach ($explorerFiles as $row) {
            ...
            echo '<tr class="cc-explorer-row" draggable="true" data-file-key="' . esc_attr($fileKey) . '">';
            ...
            echo '<td>' . self::renderExplorerActionButtons($fileKey) . '</td>';
            echo '</tr>';
        }
    ...
    private static function renderExplorerActionButtons(string $fileKey): string
    {
        $buttons = [
        ...
        foreach ($buttons as $action => $label) {
            $html .= '<button type="button" class="button button-secondary cc-inline-action" data-file-key="' . esc_attr($fileKey) . '" data-sim-action="' . esc_attr($action) . '">' . esc_html($label) . '</button>';
        }
    ...
    private static function buildExplorerDetailPanel(array $explorerFiles, array $previewRows, string $selectedProvider, ?array $selectedConnection): array
    {
        ...
        foreach ($explorerFiles as $index => $file) {
            ...
            $detail = [
                ...
                'readonly' => (bool) ($preview['status'] ?? '') === 'readonly',
                'blocked' => (bool) ($file['status'] === 'offline'),
            ];
            $map[$key] = $detail;
        }
    ...
    private static function renderExplorerScript(array $detailMap, array $previewRows, array $conflictRows, array $health, array $activityEntries): void
    {
        ...
        echo 'const updateDetail=function(key){if(!detailMap[key]){return;} state.activeKey=key; detailRoot.innerHTML=renderTable(detailMap[key]); setActiveRow(key);};';
        ...
        echo 'document.querySelectorAll(".cc-explorer-row").forEach(function(row){row.addEventListener("dragstart",function(event){const key=row.getAttribute("data-file-key")||""; if(!detailMap[key]){event.preventDefault();return;} state.activeKey=key; event.dataTransfer.setData("text/plain",key); event.dataTransfer.effectAllowed="move"; row.style.opacity="0.55"; row.style.background="#f0f6fc"; showFeedback(detailMap[key],"Safe-Mode Drop-Zone","preview",false);}); row.addEventListener("dragend",function(){row.style.opacity="1"; row.style.background=""; setActiveRow(state.activeKey); document.querySelectorAll(".cc-drop-zone").forEach(function(zone){zone.style.outline="none"; zone.style.backgroundColor="";});});});';
